...
import com.example.springapp.service.abuStudentService;


@RestController
public class abuStudentController{
    @Autowired
    abuStudentService studentService;
    @PostMapping("/student")
    public ResponseEntity<abuStudent> addStudents(@RequestBody abuStudent student)
    {
        return new ResponseEntity<>(studentService.addStudents(student),HttpStatus.CREATED);
    }
    
    @GetMapping("/students-inner-join")
    public ResponseEntity<List<abuStudent>>getStudentsInner()
    {
        return new ResponseEntity<>(studentService.getStudentInner(),HttpStatus.OK);
    }
    @GetMapping("/students-left-outer-join")
    public ResponseEntity<List<abuStudent>> getStudentsLeftOuter()
    {
